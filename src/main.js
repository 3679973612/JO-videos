...
ipcMain.handle('app:get-asset', (_e, name) => path.join(__dirname, '..', 'assets', name));
...
ipcMain.handle('recording:started', () => {
  if (hideFromCapture && mainWindow) mainWindow.setContentProtection(true);
  return true;
});
...
ipcMain.handle('recording:save', async (_e, { base64, fileName }) => {
  ...
  fs.writeFileSync(tempWebm, Buffer.from(base64, 'base64'));
...
ipcMain.handle('recording:transcode', async (_e, { inputPath, outputPath }) => {
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      ff.on('error', reject);
      ff.on('close', code => { if (code === 0) resolve(); else reject(new Error(err || 'ffmpeg code ' + code)); });
    });
  } catch (e) { throw e; }
  try {
